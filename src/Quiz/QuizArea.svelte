    ...
    let questions = [
		{
			...
			"correctIndex": 4,
            "status": false,
            "description": "The correct answer for this question is underscore becuase only underscore is allowed in variable name."
		},
		{
            "id": 2,
		...
		},
		{
			...
			"correctIndex": 1,
            "status": false,
            "description": "The correct answer for this question is memory.h because malloc and calloc functions are define in this header file."
		},
		{
            "id": 4,
...
<div class="application">
    ...
    {:else if !(questionPointer > answers.length-1)}
        <div class="quiz_screen">
            <div class="main">
                <h2>
                    QUESTION {questions[questionPointer].id} : {questions[questionPointer].question}
                </h2>
                <div class="option">
                    ...
                    <ul>
                        <li>
                            <label class="container">
                                <input type="radio" on:click={() => {selectAnswer(i)}}  name="options" disabled={showReview} checked={answers[questionPointer]==null ? false : showReview ? false : 'checked'}/>{"    " + opt}
                ...
                {#if showReview}
                <h2 class="{questions[questionPointer].correctIndex == answers[questionPointer] ? 'correct' : 'incorrect'}">
                    ...
                    {/if}
                    <span>YOUR ANSWER : {answers[questionPointer]}</span>
                </h2>
                <div class="description">
                    <h2>CORRECT ANSWER : {questions[questionPointer].correctIndex}</h2>
                    <h3>DESCRIPTION : {questions[questionPointer].description}</h3>
                </div>
                {/if}
            </div>
            <Footer />
    ...
    {:else}
        <div class="score_screen">
            ...
            {:else}
                <div class="greeting">
                    <h1><i class='bx bxs-smile bx-tada' ></i><span>Better luck next time!</span></h1>
                </div>
            {/if}
        </div>
    {/if}
